...
export default function WhatsAppWidget() {
  ...
  // Muestra el tooltip después de 5 segundos
  useEffect(() => {
    const t1 = setTimeout(() => setVisible(true), 1000)
    const t2 = setTimeout(() => setShowTooltip(true), 5000)
    const t3 = setTimeout(() => setShowTooltip(false), 12000)
  ...
  return (
    <div className="fixed bottom-6 right-4 z-50 flex flex-col items-end gap-2">
      {/* Tooltip */}
      {showTooltip && (
        <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-3.5 max-w-[220px] relative animate-fade-in-up">
          <button
          ...
          <p className="text-xs font-semibold text-navy mb-1">¿Necesita ayuda?</p>
          <p className="text-xs text-slate-500 leading-relaxed">
            Chatee con un asesor y cotice su plan en menos de 1 minuto.
          </p>
          {/* Triángulo apuntando al botón */}
          <div className="absolute -bottom-2 right-6 w-4 h-2 overflow-hidden">
            <div className="w-3 h-3 bg-white border-slate-100 border-b border-r rotate-45 translate-y-[-7px] translate-x-[2px]" />
          </div>
        </div>
      )}
